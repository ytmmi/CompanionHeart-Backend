...
KEY_DESCRIPTIONS = {
    # ── LLM ──
    "llm.mode": "当前激活的 LLM 引擎模式（openai / ollama / anthropic）",
    ...
    "llm.anthropic.api_key": "Anthropic API 密钥",
    "llm.anthropic.base_url": "Anthropic API 端点地址（可覆盖为代理/中转地址）",
    "llm.anthropic.model": "Anthropic 模型 ID（如 claude-sonnet-4-6）",
    "llm.anthropic.timeout": "请求超时时间（秒）",
    "llm.anthropic.default_params.temperature": "生成温度，越高输出越随机（0~2）",
    "llm.anthropic.default_params.max_tokens": "单次回复最大 token 数（Anthropic API 必填参数）",
    "llm.anthropic.default_params.top_p": "核采样阈值（0~1）",
    "llm.anthropic.system_prompt": "系统提示词，定义 AI 角色行为（通过 Anthropic 请求体 system 字段发送）",
