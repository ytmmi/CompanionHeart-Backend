...
def _llm_group(llm: dict) -> dict:
    return {
        ...
        "hint": "对话推理引擎。支持 OpenAI 兼容、Ollama 本地推理、Anthropic 三种格式。",
        "fields": [
            ...
            _f("llm.anthropic.api_key", "API 密钥", "password", dig(llm, "anthropic.api_key", ""),
               showIf={"key": "llm.mode", "equals": "anthropic"},
               placeholder="sk-ant-..."),
            _f("llm.anthropic.base_url", "API 端点", "text", dig(llm, "anthropic.base_url", ""),
               showIf={"key": "llm.mode", "equals": "anthropic"},
               placeholder="https://api.anthropic.com"),
            _f("llm.anthropic.model", "模型", "model", dig(llm, "anthropic.model", ""),
               showIf={"key": "llm.mode", "equals": "anthropic"}, modelSource="anthropic"),
            _f("llm.anthropic.timeout", "请求超时（秒）", "number", dig(llm, "anthropic.timeout", 120),
               showIf={"key": "llm.mode", "equals": "anthropic"}, min=1, max=600, step=1),
            ...
            _f("llm.anthropic.default_params.temperature", "生成温度", "slider",
               dig(llm, "anthropic.default_params.temperature", 0.7),
               showIf={"key": "llm.mode", "equals": "anthropic"}, min=0, max=2, step=0.05),
            _f("llm.anthropic.default_params.max_tokens", "最大输出 token（必填）", "number",
               dig(llm, "anthropic.default_params.max_tokens", 2048),
               showIf={"key": "llm.mode", "equals": "anthropic"}, min=1, max=65536, step=1),
            _f("llm.anthropic.default_params.top_p", "核采样 top_p", "slider",
               dig(llm, "anthropic.default_params.top_p", 0.95),
               showIf={"key": "llm.mode", "equals": "anthropic"}, min=0, max=1, step=0.01),
        ],
        "test": "llm",
    }
